...
func ParseWeakAreasFromGeminiResponse(response string) ([]WeakArea, error) {
	...
	// If JSON parsing failed or no valid weak areas found, try to extract weak areas using regex
	log.Println("Failed to parse JSON or no valid weak areas found, trying regex extraction")
	
	// This is a fallback method that tries to extract weak areas from the text
	// It's less reliable but can work if the AI doesn't return proper JSON
	weakAreas := []WeakArea{}
	
	// Look for patterns like "Text: ... Page: ... X: ... Y: ... Width: ... Height: ... Description: ..."
	...
	if len(weakAreas) == 0 {
		// If we still couldn't extract weak areas, log an error
